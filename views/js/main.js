...
// local function to achieve batch update of all sliding pizzas
  function calculator(){
	...
	for (var i = 0; i < items.length; i++) {
	  ...
	  //item.style.left = basicLeft + 100 * phase + 'px'; // preforms slightly worse than translateX
  	}
  }
  calculator();
...
// Generates the sliding pizzas when the page loads. // But the screen doesnt need to show 200!!!
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //var basicLeft = (i % cols) * s;
  for (var i = 0; i < 80; i++) { // reduce from 200 to 80 ???
    var elem = document.createElement('img');
    elem.className = 'mover';
    elem.src = "images/pizza_small.png";
    elem.style.height = "93px";
    elem.style.width = "72px";
    elem.basicLeft = (i % cols) * s; //basicLeft[i]; // (i % cols) * s;
